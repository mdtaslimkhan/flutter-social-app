...
class SoundRecorder{
  ...
  Future _stopRecord({String rootDir, String senderId, UserModel user, String chatType, FileUploadProvider fp, int role}) async{
    if(_audioRecorder != null) {
  ...
  Future toggleRecording({String rootDir, String senderId, UserModel user, String chatType, FileUploadProvider fp, int role, AudioProvider ap}) async {
    if(_audioRecorder != null) {
      if(_audioRecorder.isStopped){
        await _startRecord();
        ap.setIsRecordingState(isRecording: true);
      }else{
        await _stopRecord(rootDir: rootDir, senderId: senderId, user: user, chatType: chatType, fp: fp, role: role);
