import 'dart:ui';
import 'package:chat_app/ui/pages/login/otp_screen.dart';
...
class _RegisterState extends State<Register> with SingleTickerProviderStateMixin {
  ...
  void initState() {
   ...
   // userClear();
    ...
    //     _animationController.reverse(from: 2);
                             ...
                             child: Column(
                               ...
                               children: [
                                 TextFormField(
                                   ...
                                   decoration: const InputDecoration(labelText: '1917XXXXXX'),
                         ...
                         child: ElevatedButton(
                           onPressed: !isChecked ? null : () async {
                             if(_formKey.currentState.validate()){
                               final String nNumber = numberControllers.text.trim();
                               showDialog(
                                   context: context,
                                 //  barrierDismissible: false,
                                   builder: (context) {
                                     return AlertDialog(
                                       title: Text("Are you sure,You want to verify this number ${crountryCode + nNumber}?", style: TextStyle(
                                           fontSize: 14,
                                           fontFamily: 'Segoe'
                                       ),),
                                       content: Text("We will send you a verification pin to this number. If the number is right then tap on Yes, if wrong then tap no and enter correct number.",
                                       style: TextStyle(
                                         fontSize: 14,
                                         fontFamily: "Segoe"
                                       ),),
                                       actions: <Widget>[
                                         TextButton.icon(
                                           ...
                                           onPressed: () async{
                                             ...
                                             // if country is not bangladesh then run code
                                             if(smsBangladesh){
                                               // int code = await createNewCode(nNumber: crountryCode + nNumber);
                                               Navigator.push(context, MaterialPageRoute(builder: (context) => OtpScreen(number: crountryCode + nNumber,isNative: true)));
                                             }else{
                                               // await loginUser(crountryCode + nNumber, co453461ntext);
                                               Navigator.push(context, MaterialPageRoute(builder: (context) => OtpScreen(number: crountryCode + nNumber, isNative: false)));
                                             }
                                           },
                                         ),
                                         TextButton.icon(
  ...
  _launchURL() async {
    const url = 'https://circleofrectitude.com/privacy-policy';
