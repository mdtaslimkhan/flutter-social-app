...
class FollowingList extends StatefulWidget {
  ...
  final String userId;
  FollowingList({this.user , this.userId});
  @override
...
class _FollowingListState extends State<FollowingList> {
  ...
  Future getFollowereduserList({@required String uid}) async {
    try{
    final String url = BaseUrl.baseUrl("followingUserList/$uid");
  ...
  getMoreData() async{
    ...
    print(_addPage);
    try {
      final http.Response response = await http.get(Uri.parse(BaseUrl.baseUrl("followingUserList/${widget.userId}?page=$_addPage")),
          headers: {'test-pass': ApiRequest.mToken});
      var str = jsonDecode(response.body);
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text("Users I am following"),
        ),
        body: Container(
  ...
  groupItem(BuildContext context, UserItem gm, int index){
   ...
   removeFolloweredList(BuildContext context, int userid, int index) {
    print(userid);
    return showDialog(
        context: context,
        builder: (context){
          return SimpleDialog(
               children: [
                 Container(
                   ...
                   child: Column(
                     children: [
                       Text("Do you want to unfollow this user?",style: fldarkgrey15,),
                       SizedBox(height: 10,),
                       MaterialButton(
                          child: Text("Yes", style: ftwhite18,),
                           ...
                           onPressed: (){
                            ...
                            print(userid);
                             ...
                             unFollowerRequest(client: userid.toString(), myid: widget.user.id.toString());
                            setState(() {
                            });
                          },
                       )
