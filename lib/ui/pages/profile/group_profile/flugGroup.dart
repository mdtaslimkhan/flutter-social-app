...
class FlagGroup extends StatefulWidget {

  final profileId;
  final userId;
  FlagGroup({this.profileId, this.userId});

  @override
  _FlagGroupState createState() => _FlagGroupState();
}

class _FlagGroupState extends State<FlagGroup> {
 ...
 getButtonValue(state){
  ...
  _submitFlag() async {
   ...
   setUserFlag({String uId, String currentUserId, String flagstr}) async {
    try {
      final String url = BaseUrl.baseUrl("flagGroup");
      final http.Response rs = await http.post(Uri.parse(url),
          headers: { 'test-pass': ApiRequest.mToken},
          body: {'user_id' : uId, 'reson' : flagstr, 'group_id': currentUserId}
      );
  ...
  Widget build(BuildContext context) {
    return  SafeArea(
      child: Scaffold(
        appBar: AppBar(title: Text("Flag group"),
